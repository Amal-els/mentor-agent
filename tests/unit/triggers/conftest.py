...
@pytest.fixture
def ensure_test_user(pg_session):
    # get-or-create, like seed_fixture._ensure_user — these tests share a
    # database across runs (no per-test rollback), so a fixed id/slack_user_id
    # must be idempotent rather than a raw insert.
    def _ensure(user_id: str, slack_user_id: str | None = None) -> User:
        row = pg_session.get(User, user_id)
        if row is None:
            row = User(id=user_id, created_at=datetime.datetime.now(datetime.UTC))
            pg_session.add(row)
